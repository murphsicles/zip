...
pub fn WalletOverview() -> Element {
    ...
    use_effect(to_owned![wallet, paymail, user_id, balance, balance_converted, currency, primary_paymail, error, notification], || async move {
    ...
    fade_in(cx, rsx! {
        div {
            ...
            style: "{{{global_styles()}}} .wallet-overview {{ display: flex; flex-direction: column; align-items: center; padding: 20px; gap: 15px; }} .balance-main {{ font-size: 2.5em; font-weight: bold; color: #333; }} .balance-sub {{ font-size: 1.2em; color: #666; }} .paymail {{ font-size: 1.1em; color: #444; }} @media (max-width: 600px) {{ .balance-main {{ font-size: 2em; }} }} {animated}",
