...
pub fn global_styles() -> String {
    r#"
        ...
        .history-grid { display: grid; grid-template-columns: 100px 120px 140px 200px 140px 200px; gap: 10px; overflow-y: auto; max-height: 80vh; font-size: 14px; padding: 10px; }
        .header { font-weight: bold; background-color: #f0f0f0; }
        .delta-positive { color: green; }
        .delta-negative { color: red; }
        .txid-link { color: #007bff; text-decoration: none; }
        ...
        @media (max-width: 600px) { .swipe-button { width: 100%; } .history-grid { grid-template-columns: 1fr; font-size: 12px; } .dashboard { padding: 10px; } .balance-main { font-size: 2em; } }
    "#.to_string()
}
