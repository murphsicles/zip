...
impl PasskeyManager {
    ...
    pub fn start_registration(&self, user_id: Uuid, username: &str) -> Result<(CreationChallengeResponse, PasskeyRegistrationState), ZipError> {
    ...
    pub fn complete_registration(
        ...
        let serialized = bincode::serialize(&passkey).map_err(|e| ZipError::Passkey(WebauthnError::Other(e.to_string())))?;
    ...
    pub async fn start_authentication(&self, user_id: Uuid, totp_code: Option<&str>) -> Result<(RequestChallengeResponse, PasskeyAuthenticationState), ZipError> {
            ...
            if let Some(secret) = prefs.get("2fa_enabled") {
                ...
                if let Some(code) = totp_code {
                    if !totp.check_current(code)? {
                        return Err(ZipError::Passkey(WebauthnError::Other("Invalid 2FA code".to_string())));
                    }
                } else {
                    return Err(ZipError::Passkey(WebauthnError::Other("2FA code required".to_string())));
                }
            }
        }

        let cached = self.storage.get_user_data(user_id)?
            .ok_or(ZipError::Passkey(WebauthnError::CredentialRetrievalError))?;
        let passkeys: Vec<Passkey> = bincode::deserialize(&cached).map_err(|e| ZipError::Passkey(WebauthnError::Other(e.to_string())))?;
